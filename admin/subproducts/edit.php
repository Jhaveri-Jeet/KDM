...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['images'])) {
        ...
        for ($i = 0; $i < count($_FILES['images']['name']); $i++) {
            ...
            $fileUploaded = move_uploaded_file($tmpFileName, pathOf("assets/uploads/subproduct-images/$fileName"));
            if (!$fileUploaded) {
    ...
    $query = "UPDATE `SubProduct` SET `Name` = ?, `Description` = ?, `CategoryId` = ?, `ProductId` = ? WHERE `Id` = ?";
    $params = [$subProductName, $productDescription, $categoryId, $productId, $id];
